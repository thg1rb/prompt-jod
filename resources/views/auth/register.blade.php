<x-guest-layout>
    ...
    <div class="p-6 pb-3 text-center">
        <h1 class="text-[22px] font-bold text-foreground tracking-tight">สร้างบัญชีใหม่</h1>
        <p class="text-sm text-text-muted mt-1">เริ่มต้นใช้งานด้วยบัญชีฟรีของคุณวันนี้</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" x-data="{ showPassword: false, showPasswordConfirm: false }" class="p-6 pt-4 space-y-4">
        ...
        <div class="relative">
            ...
            <input id="password" :type="showPassword ? 'text' : 'password'" name="password" required autocomplete="new-password" class="w-full px-4 py-2.5 bg-background border border-border rounded-xl text-foreground placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent transition-colors pr-10 text-sm">
        ...
        <div class="relative">
            ...
            <button type="button" @click="showPasswordConfirm = !showPasswordConfirm" class="absolute right-3 top-[38px] text-text-muted hover:text-foreground transition-colors">
    ...
    <div class="px-6 pb-6 text-center">
        <span class="text-sm text-text-muted">
            {{ __('มีบัญชีอยู่แล้ว?') }}
            <a href="{{ route('login') }}" class="font-semibold text-primary hover:text-primary/80 transition-colors">
                {{ __('เข้าสู่ระบบ') }}
            </a>
        </span>
    </div>
